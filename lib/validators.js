...
const SMTP_SECURITY = ['ssl', 'starttls'];
const HOST_PATTERN = /^[a-zA-Z0-9]([a-zA-Z0-9.-]*[a-zA-Z0-9])?$/;

// `password` vazio = manter a senha já salva (server.js decide); nunca é obrigatório aqui.
function validateSmtpSettingsRequest(request) {
  if (!request || typeof request !== 'object') throw new Error('Corpo JSON inválido.');
  const host = normalize(request.host);
  const port = Number(request.port);
  const security = String(request.security || '');
  const user = normalize(request.user);
  const from = normalize(request.from) || user;
  const password = String(request.password || '');
  if (!host || host.length > 200 || !HOST_PATTERN.test(host)) throw new Error('Informe o servidor SMTP (ex.: mail.invicco.com.br).');
  if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error('Informe uma porta válida (1 a 65535).');
  if (!SMTP_SECURITY.includes(security)) throw new Error('Escolha a segurança da conexão (SSL/TLS ou STARTTLS).');
  if (!user || user.length > 200) throw new Error('Informe o usuário do SMTP (normalmente o e-mail completo).');
  if (from.length > 200 || !EMAIL_PATTERN.test(from)) throw new Error('Informe um e-mail remetente válido.');
  if (password.length > 500) throw new Error('Senha longa demais.');
  return { host, port, security, user, from, password };
}

// Conversa do Assistente salva (tela do chat). A pergunta nova de até 2000 caracteres é validada em
// /api/assistant; aqui as respostas da IA também precisam caber, então o teto é o do histórico (8000).
function validateAssistantChatRequest(request) {
  if (!request || typeof request !== 'object') throw new Error('Corpo JSON inválido.');
  const id = request.id === undefined || request.id === null ? undefined : String(request.id);
  if (id !== undefined && !/^[a-f0-9]{24}$/i.test(id)) throw new Error('Conversa inválida.');
  if (!Array.isArray(request.messages) || request.messages.length < 1 || request.messages.length > 40) {
    throw new Error('A conversa precisa ter de 1 a 40 mensagens.');
  }
  const messages = request.messages.map((m) => {
    if (!m || !['user', 'assistant'].includes(m.role) || typeof m.content !== 'string') throw new Error('Mensagem inválida.');
    const content = m.content.trim();
    if (!content || content.length > 8000) throw new Error('Cada mensagem deve ter de 1 a 8000 caracteres.');
...
module.exports = {
  ...
  validateAuthRequest, validateEmailRequest, validateResetRequest, validateSmtpSettingsRequest, validateAssistantChatRequest,
