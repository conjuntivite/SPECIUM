...
const TEMPLATE_HEADER = ['Categoria (codigo)', 'Marca', 'Modelo', '', 'Codigo', 'Grupo', 'Categoria'];
...
function parseProductImportCsv(csvText, categories) {
  ...
  dataRows.forEach((cols, index) => {
    ...
    const [rawCategory, rawBrand, rawModel] = cols;
    ...
    try {
      products.push(validateProductRequest({ category: categoryValue, brand: rawBrand, model: rawModel }));
    } catch (error) {
