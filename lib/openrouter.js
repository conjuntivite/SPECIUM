...
// `configured`: 1 modelo ou vários separados por vírgula (o 1º é o preferido, os demais são reserva
// paga); todos vêm antes da lista gratuita, que só entra se os configurados falharem.
function candidateModels(configured = process.env.OPENROUTER_MODEL) {
  const picked = (configured || '').split(',').map((m) => m.trim()).filter(Boolean);
  return picked.length ? [...picked, ...DEFAULT_MODELS.filter((m) => !picked.includes(m))] : DEFAULT_MODELS;
}
...
async function attemptOnce(model, messages, temperature, apiKey, maxTokens, reasoning) {
  const res = await fetch(OPENROUTER_URL, {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ model, messages, temperature, ...(maxTokens ? { max_tokens: maxTokens } : {}), ...(reasoning ? { reasoning } : {}) }),
  });
...
// compatível).
async function callOpenRouter({ messages, temperature = 0.2, models = candidateModels(), maxTokens, reasoning } = {}) {
  ...
  for (const model of models) {
    for (let attempt = 1; attempt <= RETRIES_PER_MODEL; attempt++) {
      try {
        const { content, usage } = await attemptOnce(model, messages, temperature, apiKey, maxTokens, reasoning);
...
module.exports = { callOpenRouter, candidateModels };
