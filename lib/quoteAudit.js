...
async function classifyQuoteItems(pdfText, categories) {
  ...
  const { content } = await callOpenRouter({ messages: [{ role: 'user', content: prompt }], models: stageModels('OPENROUTER_MODEL_CLASSIFY'), reasoning: NO_REASONING });
...
function buildAuditPrompt(quoteSummary, engineFindingsSummary, knowledge = '') {
  return [
    ...
    '- NÃO cite nem reforce as pendências que o motor de regras já listou acima (nada de "já foi apontado, mas...") — o vendedor já vê essa lista. Só o que for NOVO.',
    '',
    ...
    'Se não achar nenhum problema real além do que o motor já achou, diga isso claramente em vez de forçar uma crítica. Responda em português, direto pro vendedor, sem introdução nem conclusão, sem inventar tabela nem JSON.',
  ].join('\n');
}
...
async function auditQuoteWithAI(items, categories, engineMissing) {
  ...
  const { content } = await callOpenRouter({ messages: [{ role: 'user', content: buildAuditPrompt(quoteSummary, engineFindingsSummary, knowledge) }], temperature: 0.4, maxTokens: AUDIT_MAX_OUTPUT_TOKENS, models: stageModels('OPENROUTER_MODEL_AUDIT'), reasoning: NO_REASONING });
  return content.trim();
}
