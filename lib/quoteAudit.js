...
function buildAuditPrompt(quoteSummary, engineFindingsSummary) {
  return [
...
async function auditQuoteWithAI(items, categories, engineMissing) {
  ...
  const { content } = await callOpenRouter({ messages: [{ role: 'user', content: buildAuditPrompt(quoteSummary, engineFindingsSummary) }], temperature: 0.4 });
  return content.trim();
}
