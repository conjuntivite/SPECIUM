...
// Mesmo formato de serveStatic, mas apontado pra um diretório qualquer (ex: uploads/) — usado pra
// servir a planta baixa enviada pelo consultor, que fica fora de STATIC_DIRECTORY de propósito.
function serveFromDirectory(directory, requestPath, response) {
  const filePath = path.resolve(directory, requestPath);
  if (!filePath.startsWith(`${directory}${path.sep}`)) return sendJson(response, 403, { detail: 'Acesso negado.' });
  fs.readFile(filePath, (error, file) => {
    if (error) return sendJson(response, 404, { detail: 'Arquivo não encontrado.' });
    response.writeHead(200, { 'Content-Type': MIME_TYPES[path.extname(filePath)] || 'application/octet-stream' });
    response.end(file);
...
// Corpo binário cru (upload de arquivo) — mesmo padrão de readJson (acumula e limita), mas devolve
// Buffer em vez de fazer JSON.parse.
function readBinary(request, maxBytes) {
  return new Promise((resolve, reject) => {
    ...
    request.on('data', (chunk) => {
      size += chunk.length;
      if (size > maxBytes) { request.destroy(); reject(new Error('Arquivo excede o tamanho máximo permitido.')); return; }
      chunks.push(chunk);
    });
...
module.exports = { sendJson, sendCsv, readJson, readBinary, serveStatic, serveFromDirectory, parseCookies };
